<template>
  <el-drawer title="列展示" class="drawer-columns-filter" :visible.sync="active" size="300px" append-to-body>
    <q7-drawer-container class="hy-table-columns-filter">
      <!-- 全选 反选 -->
      <div class="flex justify-between items-center mb-4">
        <el-checkbox :indeterminate="isIndeterminate" v-model="checkAll" @change="onCheckAllChange"> {{ showLength }} / {{ options.length }} </el-checkbox>
        <el-tag>固定列</el-tag>
      </div>
      <el-card shadow="never">
        <draggable ghost-class="ghost" class="component--list" handle=".handle" v-model="currentValue">
          <transition-group>
            <div v-for="(option, index) of currentValue" :key="option.id" class="component--list-item">
              <el-checkbox v-model="currentValue[index].show" class="flex-1">
                {{ option.label || option.prop || '未命名' }}
              </el-checkbox>
              <hy-table-columns-fixed-controller class="mr-2 flex-shrink-0" v-model="currentValue[index].fixed" @change="(value) => fixedChange(index, value)" />
              <div class="component--list-item-handle handle flex-shrink-0">
      ...
      <el-row slot="footer" :gutter="10">
        <el-col :span="12">
          <q7-button size="default" icon="el-icon-refresh" label="重置" block @click="reset" />
        </el-col>
        <el-col :span="12">
          <q7-button size="default" type="primary" icon="el-icon-check" label="确定" block @click="submit" />
        </el-col>
      </el-row>
    </q7-drawer-container>
  </el-drawer>
...
.drawer-columns-filter /deep/ .el-drawer__header {
  margin-bottom: 20px;
}
