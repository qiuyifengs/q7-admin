...
export default {
  mixins: [ OptionSizeMixin, RefocusTargetMixin, FormMixin ],
  ...
  computed: {
    ...
    classes () {
      return `q7-${this.type} cursor-pointer no-outline row inline no-wrap items-center` +
        (this.disable === true ? ' disabled' : '') +
        (this.dense === true ? ` q7-${this.type}--dense` : '') +
    ...
    innerClass () {
      ...
      return `q7-${this.type}__inner--${state}${color}`
    },
  ...
  render (h) {
    ...
    this.disable !== true && this.__injectFormInput(
      inner,
      'unshift',
      `q7-${this.type}__native absolute`
    )

    const child = [
      h('div', {
        staticClass: `q7-${this.type}__inner relative-position non-selectable`,
    ...
    label !== void 0 && child.push(
      h('div', {
        staticClass: `q7-${this.type}__label q-anchor--skip`
      }, label)
    )
